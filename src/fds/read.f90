...
USE MESH_POINTERS
USE OUTPUT_DATA
USE COMP_FUNCTIONS, ONLY: SECOND, CHECKREAD, SHUTDOWN, CHECK_XB, SCAN_INPUT_FILE
...
IMPLICIT NONE
PRIVATE
CHARACTER(255), PARAMETER :: readid='$Id: read.f90 22279 2015-04-07 14:57:50Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: readrev='$Revision: 22279 $'
CHARACTER(255), PARAMETER :: readdate='$Date: 2015-04-07 07:57:50 -0700 (Tue, 07 Apr 2015) $'
...
CALL PROC_REAC    ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
CALL READ_VERT    ; IF (STOP_STATUS==SETUP_STOP) RETURN
CALL READ_FACE    ; IF (STOP_STATUS==SETUP_STOP) RETURN
CALL READ_CSVF    ; IF (STOP_STATUS==SETUP_STOP) RETURN
CALL READ_VOLU    ; IF (STOP_STATUS==SETUP_STOP) RETURN
...
CALL PROC_DEVC    ; IF (STOP_STATUS==SETUP_STOP) RETURN
   ...
   K_MULT_LOOP: DO KK=MR%K_LOWER,MR%K_UPPER
      J_MULT_LOOP: DO JJ=MR%J_LOWER,MR%J_UPPER
         I_MULT_LOOP: DO II=MR%I_LOWER,MR%I_UPPER
            ...
            IF (M%MESH_LEVEL>0) EMBEDDED_MESH=.TRUE.
...
IF (USE_MPI) THEN
   ...
   DO NM=1,NMESHES
      ...
      IF (PROCESS(NM) < PROCESS(NM-1)) THEN
         WRITE(MESSAGE,'(A,I3,A,I3,A)') 'ERROR: MPI_PROCESS for evacuation MESH ', NM,' < MPI_PROCESS for MESH ',NM-1,&
                                        '. Reorder MESH lines.'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF
   ENDDO
...
JMAX = 0
DO I = 1, NM
   EV_IF: IF (EVACUATION_GRID(I) .AND. EVACUATION_ONLY(I)) THEN
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   ELSE
      ...
      M%DT = M%CELL_SIZE/VEL_CHAR*CFL_MAX
   ENDIF
...
COUNT_MULT_LOOP: DO
   ...
   N_MULT = N_MULT + 1
   10 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I2,A,I5)') 'ERROR: Problem with MULT number',N_MULT,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
USE GPYRO_VARS, ONLY: GPG !CWL
USE MATH_FUNCTIONS, ONLY: GET_RAMP_INDEX
REAL(EB) :: CORIOLIS_VECTOR(3)=0._EB,FORCE_VECTOR(3)=0._EB,MAXIMUM_VISIBILITY
CHARACTER(LABEL_LENGTH) :: ASSUMED_GAS_TEMPERATURE_RAMP,EXTINCTION_MODEL,RAMP_GX,RAMP_GY,RAMP_GZ,&
                           RAMP_U0,RAMP_V0,RAMP_W0,RAMP_U0_Z,RAMP_V0_Z,RAMP_W0_Z,TURBULENCE_MODEL
INTEGER :: NUM_GPYRO_MESHES = 1, IGPYROMESH, PROCESS_GPYRO(1:100) !CWL
REAL(EB), DIMENSION(1:100,1:3) :: GPYRO_MESH_CENTROIDS, GPYRO_MESH_EXTENTS !CWL
...
NAMELIST /MISC/ AL2O3,ALLOW_SURFACE_PARTICLES,ALLOW_UNDERSIDE_PARTICLES,ASSUMED_GAS_TEMPERATURE,ASSUMED_GAS_TEMPERATURE_RAMP,&
                BAROCLINIC,BNDF_DEFAULT,CNF_CUTOFF,CFL_MAX,CFL_MIN,CFL_VELOCITY_NORM,&
                CHECK_HT,CHECK_REALIZABILITY,CHECK_VN,CLIP_MASS_FRACTION,&
                COMPUTE_VISCOSITY_TWICE,CONSTANT_SPECIFIC_HEAT_RATIO,CORIOLIS_VECTOR,&
                C_DEARDORFF,C_RNG,C_RNG_CUTOFF,C_SMAGORINSKY,C_VREMAN,&
                DNS,DT_MEAN_FORCING,ENTHALPY_TRANSPORT,&
                EVACUATION_DRILL,EVACUATION_MC_MODE,EVAC_PRESSURE_ITERATIONS,EVAC_SURF_DEFAULT,EVAC_TIME_ITERATIONS,&
                EXTINCTION_MODEL,PARTICLE_CFL_MAX,PARTICLE_CFL_MIN,PARTICLE_CFL,&
                FDS5_OPTIONS,FLUX_LIMITER,FORCE_VECTOR,FREEZE_VELOCITY,FYI,GAMMA,GRAVITATIONAL_DEPOSITION,&
                GRAVITATIONAL_SETTLING,GROUND_LEVEL,GVEC,HRRPUV_AVERAGE,HRRPUV_MAX_SMV,HRRPUA_SHEET,HUMIDITY,&
                IMMERSED_BOUNDARY_METHOD,INITIAL_UNMIXED_FRACTION,ISOTROPIC_PARTICLE_FORCE,&
                KINETIC_ENERGY_SOURCE,LAPSE_RATE,LES_FILTER_WIDTH,MAX_CHEMISTRY_ITERATIONS,&
                MAX_LEAK_PATHS,MAXIMUM_VISIBILITY,MEAN_FORCING,MPI_TIMEOUT,NEW_OPEN_BOUNDARY,NOISE,NOISE_VELOCITY,NO_EVACUATION,&
                OVERWRITE,PERIODIC_TEST,POROUS_FLOOR,POTENTIAL_TEMPERATURE_CORRECTION,PR,PROJECTION,P_INF,&
                RAMP_GX,RAMP_GY,RAMP_GZ,RAMP_U0,RAMP_V0,RAMP_W0,RAMP_U0_Z,RAMP_V0_Z,RAMP_W0_Z,&
                RADIATION,RESEARCH_MODE,RESTART,RESTART_CHID,RICHARDSON_ERROR_TOLERANCE,RUN_AVG_FAC,SC,&
                ...
                THERMOPHORETIC_DEPOSITION,THICKEN_OBSTRUCTIONS,TMPA,TURBULENCE_MODEL,TURBULENT_DEPOSITION,TURB_INIT_CLOCK,&
                U0,USE_NEW_CHECK_MASS_FRACTION,UVW_FILE,V0,&
                ...
                TIGNSTART,TIGNSTOP,IGNLOC,IGNPOW,SKIP_FDS_TIMESTEPS,NSKIP_FDS_TIMESTEPS,TSTART_SKIP_FDS_TIMESEPS, & !CWL
                NUM_GPYRO_MESHES, GPYRO_MESH_CENTROIDS, GPYRO_MESH_EXTENTS, PROCESS_GPYRO !CWL
...
RAMP_GY              = 'null'
RAMP_GZ              = 'null'
DT_MEAN_FORCING      = 1.E10_EB     ! must be set by user
...
!BEGIN CWL ADDITION
GPG%NUM_GPYRO_MESHES = NUM_GPYRO_MESHES
GPG%PROCESS_GPYRO(1:100) = PROCESS_GPYRO(1:100)
DO IGPYROMESH = 1, NUM_GPYRO_MESHES
   GPG%MESH_CENTROIDS (IGPYROMESH,1:3) = GPYRO_MESH_CENTROIDS(IGPYROMESH,1:3)
   GPG%MESH_EXTENTS   (IGPYROMESH,1:3) = GPYRO_MESH_EXTENTS  (IGPYROMESH,1:3)
ENDDO
!END CWL ADDITION
...
IF (RAMP_U0/='null') CALL GET_RAMP_INDEX(RAMP_U0,'TIME',I_RAMP_U0)
IF (RAMP_V0/='null') CALL GET_RAMP_INDEX(RAMP_V0,'TIME',I_RAMP_V0)
IF (RAMP_W0/='null') CALL GET_RAMP_INDEX(RAMP_W0,'TIME',I_RAMP_W0)
...
IF (ANY(MEAN_FORCING)) THEN
   IF (DT_MEAN_FORCING>1.E9_EB) THEN
      WRITE(MESSAGE,'(A)')  'ERROR on MISC: MEAN_FORCING requires DT_MEAN_FORCING'
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ENDIF
...
DT_FLUSH     = -1._EB
DT_DEVC      = -1._EB
DT_DEVC_LINE = 0.5_EB*(T_END-T_BEGIN)
DT_HRR       = -1._EB
DT_ISOF      = -1._EB
DT_MASS      = -1._EB
DT_PART      = -1._EB
...
INTEGER  :: N_SPEC_READ,N,NN,NNN,NS2,NR,N_SPEC_READ_2,N_SUB_SPECIES,NS,N_BINS
INTEGER, ALLOCATABLE, DIMENSION(:) :: Y_INDEX
LOGICAL  :: LUMPED_COMPONENT_ONLY,AEROSOL,BACKGROUND,&
            DEFINED_BACKGROUND,REAC_FUEL_READ=.FALSE.,PRIMITIVE
...
COUNT_SPEC_LINES: DO
   ...
   IF (N_PARTICLE_BINS > 0) THEN
      MINT_SPECIES=MINT_SPECIES+N_PARTICLE_BINS
   ENDIF
20 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I2,A,I5)') 'ERROR: Problem with SPECies number',N_SPECIES+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
...
PRIMITIVE_SPEC_READ_LOOP: DO WHILE (N_SPEC_READ < N_SPEC_READ_2 .OR. N < N_SPECIES)
   ...
   IF (N_BINS > 0) THEN
      IF (AGGLOMERATION) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': Can only specify N_BINS for one &SPEC input'
         CALL SHUTDOWN(MESSAGE) ; RETURN 
      ENDIF
      IF (N_BINS < 2) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': N_BINS must be >=2'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF
      IF (.NOT. AEROSOL) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': AEROSOL must be .TRUE. to use N_BINS'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF      
      IF (MAX_DIAMETER < 0._EB) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': MAX_DIAMETER not set'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF
      IF (MIN_DIAMETER < 0._EB) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': MIN_DIAMETER not set'
         CALL SHUTDOWN(MESSAGE) ; RETURN                  
      ENDIF      
      IF (MAX_DIAMETER <= MIN_DIAMETER) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': MAX_DIAMETER <= MIN_DIAMETER'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF      
      IF (.NOT. LUMPED_COMPONENT_ONLY) THEN
         WRITE(MESSAGE,'(A,I2,A)') 'ERROR: SPEC ',N_SPEC_READ,': LUMPED_COMPONENT_ONLY must be .TRUE. to use N_BINS'
         CALL SHUTDOWN(MESSAGE) ; RETURN         
      ENDIF
      N_PARTICLE_BINS = N_BINS
      MAX_PARTICLE_DIAMETER = MAX_DIAMETER
      MIN_PARTICLE_DIAMETER = MIN_DIAMETER
      N_TRACKED_SPECIES=N_TRACKED_SPECIES+N_PARTICLE_BINS
      AGGLOMERATION_INDEX=N
      AGGLOMERATION = .TRUE.
      SS%AGGLOMERATING = .TRUE.
      MIN_PARTICLE_DIAMETER = MIN_PARTICLE_DIAMETER * 1.E-6_EB
      MAX_PARTICLE_DIAMETER = MAX_PARTICLE_DIAMETER * 1.E-6_EB
      CALL INITIALIZE_AGGLOMERATION
   ENDIF
   ...
   SELECT CASE (ID)
      CASE('WATER VAPOR')
         ...
         ENDIF
         IF (PREDEFINED_SMIX(1)) Y_H2O_INFTY = WATER_VAPOR_MASS_FRACTION(HUMIDITY,TMPA,P_INF)
      CASE('CARBON DIOXIDE')
         CO2_INDEX = N
...
ALLOCATE(SPECIES_MIXTURE(1:N_TRACKED_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','SPECIES',IZERO)
   ...
   ELSE
      ...
      IF (N_BINS > 0) THEN
         DO NNN=1,N_PARTICLE_BINS
            ...
            SPEC_ID(1)=SPECIES(AGGLOMERATION_INDEX)%ID         
            WRITE(ID,'(A,A,I0)') TRIM(SPECIES(AGGLOMERATION_INDEX)%ID),'_',NNN
            MASS_FRACTION(1)=1._EB
            CALL DEFINE_MIXTURE
            N = N + 1
         ENDDO
         AGGLOMERATION_INDEX = N - N_PARTICLE_BINS
         N = N - 1
      ELSE
         CALL DEFINE_MIXTURE
      ENDIF
...
SELECT CASE(N)
   ...
   CASE(3)
      ...
      SPEC_ID(5)         = 'NITROGEN'
      RN%NU_CO           = (SPECIES_MIXTURE(FUEL_SMIX_INDEX)%MW/MW_CO)     *RN%CO_YIELD
...
ALLOCATE(Z2Y(N_SPECIES,1:N_TRACKED_SPECIES),STAT=IZERO)
...
ALLOCATE(MWR_Z(1:N_TRACKED_SPECIES),STAT=IZERO)
CALL ChkMemErr('READ','MW_AVG_Y',IZERO)

MWR_Z(1:N_TRACKED_SPECIES) = 1._EB/SPECIES_MIXTURE(1:N_TRACKED_SPECIES)%MW
...
ALLOCATE(D_TMP_Z(1:N_TRACKED_SPECIES))
D_TMP_Z = -1.E30_EB
ALLOCATE(MU_TMP_Z(1:N_TRACKED_SPECIES))
MU_TMP_Z = -1.E30_EB
ALLOCATE(CP_TMP_Z(1:N_TRACKED_SPECIES))
CP_TMP_Z = -1.E30_EB
ALLOCATE(H_TMP_Z(1:N_TRACKED_SPECIES))
H_TMP_Z = -1.E30_EB
ALLOCATE(K_TMP_Z(1:N_TRACKED_SPECIES))
K_TMP_Z = -1.E30_EB
ALLOCATE(G_F_TMP_Z(1:N_TRACKED_SPECIES))
G_F_TMP_Z = -1.E30_EB

ALLOCATE(CPBAR_Z(0:5000,1:N_TRACKED_SPECIES))
CALL ChkMemErr('READ','CPBAR_Z',IZERO)
CPBAR_Z = 0._EB

ALLOCATE(CP_AVG_Z(0:5000,1:N_TRACKED_SPECIES))
...
ALLOCATE(D_Z(0:5000,1:N_TRACKED_SPECIES))
CALL ChkMemErr('READ','D_Z',IZERO)
D_Z = 0._EB

ALLOCATE(G_F_Z(0:5000,1:N_TRACKED_SPECIES))
...
TABLE_LOOP: DO J=1,5000
   ...
   DO N=1,N_TRACKED_SPECIES
      SM => SPECIES_MIXTURE(N)
      CALL CALC_MIX_PROPS(J,D_TMP_Z(N),MU_TMP_Z(N),K_TMP_Z(N),CP_TMP_Z(N),H_TMP_Z(N),SM%EPSK,SM%SIG,SM%D_USER,SM%MU_USER,&
                          SM%K_USER,SM%MW,SM%SPECIFIC_HEAT,SM%REFERENCE_ENTHALPY,SM%REFERENCE_TEMPERATURE,SM%PR_USER)
      IF (SM%RAMP_CP_INDEX>0) THEN
...
REAL(EB) :: SOOT_YIELD,CO_YIELD,EPUMO2,A, &
            CRITICAL_FLAME_TEMPERATURE,HEAT_OF_COMBUSTION,E,C,H,N,O, &
            AUTO_IGNITION_TEMPERATURE,SOOT_H_FRACTION,N_T,K,NU(MAX_SPECIES),N_S(MAX_SPECIES)
REAL(EB) :: E_TMP=0._EB,S_TMP=0._EB,ATOM_COUNTS(118),MW_FUEL=0._EB,H_F,PR_TMP
...
REAC_LOOP: DO NR=1,N_REACTIONS
   ...
   ELSE ! Heat of combustion not specified
      IF (SIMPLE_CHEMISTRY .AND. RN%HEAT_OF_COMBUSTION<0._EB) THEN ! Calculate heat of combustion based oxygen consumption
...
USE MATH_FUNCTIONS, ONLY : GET_RAMP_INDEX
USE DEVICE_VARIABLES, ONLY : DEVICE, PROPERTY_TYPE
...
REAL(EB) :: DRAG_COEFFICIENT(3),FREE_AREA_FRACTION,PERMEABILITY(3),POROUS_VOLUME_FRACTION
...
NAMELIST /PART/ AGE,BREAKUP,BREAKUP_CNF_RAMP_ID,BREAKUP_DISTRIBUTION,BREAKUP_GAMMA_D,BREAKUP_RATIO,&
                ...
                QUANTITIES_SPEC_ID,RADIATIVE_PROPERTY_TABLE,&
   ...
   IF (LPC%N_ORIENTATION>0) THEN
      ...
      DO NN=1,LPC%N_ORIENTATION
         ...
         IF (ALL(ABS(OR_TEMP(1:3))<TWO_EPSILON_EB)) THEN
            WRITE(MESSAGE,'(A,I3,A)') 'ERROR: PART line ',N,'.  All components of ORIENTATION are zero.'
            CALL SHUTDOWN(MESSAGE) ; RETURN         
         ENDIF
   ...
   SELECT CASE(DRAG_LAW)
      ...
      CASE('POROUS MEDIA')
         IF (ANY(DRAG_COEFFICIENT<TWO_EPSILON_EB) .OR. ANY(PERMEABILITY<TWO_EPSILON_EB)) THEN
            WRITE(MESSAGE,'(A,I3,A)') 'ERROR: PART line ',N,&
...
COUNT_PROP_LOOP: DO
   ...
   N_PROP = N_PROP + 1
   34 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with PROP number', N_PROP+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
READ_PROP_LOOP: DO N=0,N_PROP
   ...
   PATCH_VELOCITY_IF: IF (VELOCITY_COMPONENT>0) THEN
      IF(VELOCITY_COMPONENT > 3) THEN
         WRITE(MESSAGE,'(A,A,A,I4)') 'ERROR: Problem with PROP ',TRIM(PY%ID),', VELOCITY_COMPONENT > 3: ',VELOCITY_COMPONENT
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
      IF(P0<-1.E9_EB) THEN
...
COUNT_MATL_LOOP: DO
   ...
   MATL_NAME(N_MATL) = ID
   34 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with MATL number', N_MATL+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
LOGICAL :: ADIABATIC,BURN_AWAY,FREE_SLIP,NO_SLIP,CONVERT_VOLUME_TO_MASS,GPYRO !CWL: Added GPYRO
...
REAL(EB) :: TAU_Q,TAU_V,TAU_T,TAU_MF(MAX_SPECIES),HRRPUA,MLRPUA,TEXTURE_WIDTH,TEXTURE_HEIGHT,VEL_T(2),&
            ...
            C_FORCED_CONSTANT,C_FORCED_PR_EXP,C_FORCED_RE,C_FORCED_RE_EXP,C_VERTICAL,C_HORIZONTAL,EMISSIVITY_B
...
LOGICAL :: VEGETATION,VEGETATION_NO_BURN,VEGETATION_LINEAR_DEGRAD,VEGETATION_ARRHENIUS_DEGRAD,VEG_LEVEL_SET_SPREAD,&
           DEFAULT,EVAC_DEFAULT,VEG_LSET_ELLIPSE,VEG_LSET_TAN2,TGA_ANALYSIS
NAMELIST /SURF/ ADIABATIC,BACKING,BURN_AWAY,CELL_SIZE_FACTOR,C_FORCED_CONSTANT,C_FORCED_PR_EXP,C_FORCED_RE,C_FORCED_RE_EXP,&
                ...
                HEAT_TRANSFER_MODEL,HRRPUA,ID,IGNITION_TEMPERATURE,INNER_RADIUS,INTERNAL_HEAT_SOURCE,LAYER_DIVIDE,&
                ...
                EXTERNAL_FLUX_RAMP,TAU_EXTERNAL_FLUX,VOLUME_FLUX,GPYRO ! Backwards compatability?? CWL:  Aded Gpyro
...
COUNT_SURF_LOOP: DO
   ...
   N_SURF = N_SURF + 1
   34 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with SURF number', N_SURF+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
!  SF%VEG_DEHYDRATION_RATE_MAX = SRF_VEG_DEHYDRATION_RATE_MAX
   ...
   COUNT_LAYERS: DO NL=1,MAX_LAYERS
      ...
      EMISSIVITY_B = EMISSIVITY*SF%LAYER_DENSITY(NL)
   ...
   IF (SF%EMISSIVITY_BACK < 0._EB) THEN
      IF (SF%N_LAYERS>0 .AND. SF%LAYER_DENSITY(1)>0._EB) THEN
         SF%EMISSIVITY_BACK = EMISSIVITY_B
      ELSE
         SF%EMISSIVITY_BACK = EMISSIVITY_DEFAULT
      ENDIF
   ENDIF
...
!      SF%THERMALLY_THICK = .TRUE. !CWL:  replaced this line with line below
      IF (.NOT. SF%GPYRO) SF%THERMALLY_THICK = .TRUE. !CWL
   ...
   IF (SF%GPYRO)                                    SF%THERMAL_BC_INDEX = GPYRO_BC !CWL: Added this line

   ! Boundary layer profile

   IF (SF%PROFILE==BOUNDARY_LAYER_PROFILE) THEN
      IF ( ABS(VEL_BULK)>ABS(VEL) ) THEN
         WRITE(MESSAGE,'(A,I3)') 'ERROR: SURF '//TRIM(SF%ID)// ' VEL_BULK invalid, must have VEL_BULK <= VEL'
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   ENDIF
...
GPYRO                        = .FALSE. !CWL: Added this line
...
PROCESS_SURF_LOOP: DO N=0,N_SURF
  ...
  ! Determine if the surface is combustible/burning
   ...
   IF (SF%GPYRO) SF%SPECIES_BC_INDEX = SPECIFIED_MASS_FLUX !CWL
...
!   IF (.NOT.SF%THERMALLY_THICK) THEN !CWL: Replaced this line with below:
    IF ((.NOT. SF%GPYRO) .AND. (.NOT. SF%THERMALLY_THICK)) THEN !CWL
      SF%N_LAYERS     = 1
      SF%N_CELLS_MAX  = 1
      SF%N_CELLS_INI  = 1
      SF%N_MATL   = 1
...
NAMELIST /PRES/ CHECK_POISSON,FISHPAK_BC,LAPLACE_PRESSURE_CORRECTION,MAX_PRESSURE_ITERATIONS, &
...
NAMELIST /RADI/ ANGLE_INCREMENT,BAND_LIMITS,KAPPA0,NMIEANG,NUMBER_INITIAL_ITERATIONS,NUMBER_RADIATION_ANGLES,PATH_LENGTH,&
                RADIATION,RADIATIVE_FRACTION,RADTMP,RTE_SOURCE_CORRECTION,TIME_STEP_INCREMENT,WIDE_BAND_MODEL, &
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   COUNT_OBST_LOOP: DO
      ...
      2 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I5,A,I5)')  'ERROR: Problem with OBST number',N_OBST+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
COUNT_LOOP: DO
   ...
   2 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I5,A,I5)')  'ERROR: Problem with HOLE number',N_HOLE_O+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
ENDDO COUNT_LOOP
1 REWIND(LU_INPUT) ; INPUT_FILE_LINE_NUMBER = 0
...
MESH_LOOP_1: DO NM=1,NMESHES
   ...
   COUNT_VENT_LOOP: DO
      ...
      4 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I4,A,I4)') 'ERROR: Problem with VENT ',N_VENT+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
COUNT_LOOP: DO
   ...
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with INIT number ',N_INIT_READ+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
   N_INIT_NEW = 0
   IF (MULT_ID=='null') THEN
...
COUNT_ZONE_LOOP: DO
   ...
   N_ZONE = N_ZONE + 1
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with ZONE number ',N_ZONE+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
INTEGER  :: NN,NM,MESH_NUMBER,N_DEVC_READ,IOR,TRIP_DIRECTION,VELO_INDEX,POINTS,I_POINT,PIPE_INDEX,ORIENTATION_INDEX, &
            ORIENTATION_NUMBER,STATISTICS_LOCATION_INDEX
...
NAMELIST /DEVC/ BYPASS_FLOWRATE,CONVERSION_FACTOR,COORD_FACTOR,CTRL_ID,DELAY,DEPTH,DEVC_ID,DRY,DUCT_ID,EVACUATION,FLOWRATE,FYI,&
                ...
                RELATIVE,ROTATION,SETPOINT,SMOOTHING_FACTOR,SPEC_ID,STATISTICS,STATISTICS_START,SURF_ID,&
                TIME_AVERAGED,TRIP_DIRECTION,UNITS,VELO_INDEX,XB,XYZ,X_ID,Y_ID,Z_ID
...
COUNT_DEVC_LOOP: DO
   ...
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I4,A,I5)') 'ERROR: Problem with DEVC number ',N_DEVC_READ+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
...
READ_DEVC_LOOP: DO NN=1,N_DEVC_READ
   ...
   IF (STATISTICS(1:6)=='MAXLOC' .OR. STATISTICS(1:6)=='MINLOC') THEN
      IF (STATISTICS(8:8)=='X') STATISTICS_LOCATION_INDEX = 1
      IF (STATISTICS(8:8)=='Y') STATISTICS_LOCATION_INDEX = 2
      IF (STATISTICS(8:8)=='Z') STATISTICS_LOCATION_INDEX = 3
      IF (STATISTICS(1:6)=='MAXLOC') STATISTICS = 'MAX'
      IF (STATISTICS(1:6)=='MINLOC') STATISTICS = 'MIN'
      UNITS = 'm'
   ENDIF
   ...
   POINTS_LOOP: DO I_POINT=1,POINTS
      ...
      DV%STATISTICS_LOCATION_INDEX = STATISTICS_LOCATION_INDEX
   ...
   IF ((XB(1)>-1.E5_EB.OR.STATISTICS/='null') .AND. POINTS==1) THEN
      ...
      IF (ABS(XB(1)-XB(2))<=SPACING(XB(2)) .AND. IOR==0) DV%IOR_ASSUMED = 1
      IF (ABS(XB(3)-XB(4))<=SPACING(XB(4)) .AND. IOR==0) DV%IOR_ASSUMED = 2
      IF (ABS(XB(5)-XB(6))<=SPACING(XB(6)) .AND. IOR==0) DV%IOR_ASSUMED = 3
   ENDIF
...
COUNT_CTRL_LOOP: DO
   ...
   N_CTRL = N_CTRL + 1
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I4,A,I5)') 'ERROR: Problem with CTRL number ',N_CTRL+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
...
PROC_DEVC_LOOP: DO N=1,N_DEVC
   ...
   IF (DV%LINE == 0 .AND. (DV%STATISTICS=='RMS' .OR. DV%STATISTICS=='COV' .OR. DV%STATISTICS=='CORRCOEF')) THEN
      DV%TIME_AVERAGED=.FALSE.
      IF (DV%STATISTICS_START < T_BEGIN) DV%STATISTICS_START = T_BEGIN
   ENDIF
...
COUNT_PROF_LOOP: DO
   ...
   N_PROF = N_PROF + 1
   12 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I4,A,I5)') 'ERROR: Problem with PROF number ',N_PROF+1,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
   ENDIF
...
COUNT_ISOF_LOOP: DO
   ...
   N_ISOF = N_ISOF + 1
   10 IF (IOS>0) THEN
      WRITE(MESSAGE,'(A,I2,A,I5)') 'ERROR: Problem with ISOF number ',N_ISOF,', line number',INPUT_FILE_LINE_NUMBER
      CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
MESH_LOOP: DO NM=1,NMESHES
   ...
   COUNT_SLCF_LOOP: DO
      ...
      10 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with SLCF number ',N_SLCF_O+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
      ENDIF
...
COUNT_BNDF_LOOP: DO
   ...
   N_BNDF = N_BNDF + 1
   210 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with BNDF number ',N_BNDF+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
       ENDIF
...
COUNT_BNDE_LOOP: DO
   ...
   N_BNDE = N_BNDE + 1
   310 IF (IOS>0) THEN
         WRITE(MESSAGE,'(A,I3,A,I5)') 'ERROR: Problem with BNDE number ',N_BNDE+1,', line number',INPUT_FILE_LINE_NUMBER
         CALL SHUTDOWN(MESSAGE) ; RETURN
       ENDIF
...
QUANTITY_INDEX_LOOP: DO ND=-N_OUTPUT_QUANTITIES,N_OUTPUT_QUANTITIES
   ...
   QUANTITY_IF:IF (QUANTITY==OUTPUT_QUANTITY(ND)%NAME) THEN
      ...
      IF (TRIM(QUANTITY)=='DIFFUSIVITY' .AND. DNS .AND. Y_INDEX < 0) THEN
