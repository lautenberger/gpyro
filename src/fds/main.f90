...
USE GPYRO_VARS, ONLY: IGPYRO_TYPE, GPYRO_TO_FDS_COUPLING_TYPE, GPYRO_TO_GPYRO_INTERFACE, NGPYRO_CELLS_NEEDING_BCS, GPG, GPYRO2FDS, G2GIFACE, IFACETEMP, GPM !CWL: Added this line
USE GPYRO_INIT, ONLY: ALLOCATE_GPYRO, ALLOCATE_GPYRO2FDS, INIT_GPYRO_VARS !CWL: Added this line
USE GPYRO_IO, ONLY: READ_GPYRO !CWL: Added this line

IMPLICIT NONE

! Miscellaneous declarations

CHARACTER(255), PARAMETER :: mainid='$Id: main.f90 22330 2015-04-10 18:40:12Z fds.firebot $'
CHARACTER(255), PARAMETER :: mainrev='$Revision: 22330 $'
CHARACTER(255), PARAMETER :: maindate='$Date: 2015-04-10 11:40:12 -0700 (Fri, 10 Apr 2015) $'
...
INTEGER :: ISKIP,IGPYROMESH,IRECV,IDEST,ICOUNT,LUMAP,IFROM,ITAG,ISTATUS(MPI_STATUS_SIZE),ISTART,ISTOP,ISIZE,J,IMESH1,IMESH2,IZ,IX,IY !CWL
TYPE (GPYRO_TO_FDS_COUPLING_TYPE), ALLOCATABLE, DIMENSION (:) :: MAPPING !CWL
REAL(EB) :: DELTAX, DELTAT, KINTERFACE !CWL
IGPYRO_TYPE = 2 !CWL: Added this line
...
GPG%MYID = MYID ; GPG%USE_MPI=USE_MPI !CWL
...
!CALL CHECK_MPI_OPENMP !CWL:  commented this line
...
! ENDIF
! BEGIN CWL ADDITION
!WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 01' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

IF (.NOT. USE_MPI) GPG%PROCESS_GPYRO(:) = 0

MESHES(:)%N_GPYRO_WALL_CELLS = 0
DO NM = 1, NMESHES
   IF (MYID .NE. PROCESS(NM)) CYCLE
   DO IW=1,MESHES(NM)%N_EXTERNAL_WALL_CELLS+MESHES(NM)%N_INTERNAL_WALL_CELLS
      IF ( SURFACE(MESHES(NM)%WALL(IW)%SURF_INDEX)%THERMAL_BC_INDEX == GPYRO_BC) MESHES(NM)%N_GPYRO_WALL_CELLS = MESHES(NM)%N_GPYRO_WALL_CELLS + 1
   ENDDO
   WRITE(*,*) 'Number of gpyro wall cells in mesh: ', NM, ': ', MESHES(NM)%N_GPYRO_WALL_CELLS
ENDDO

CALL READ_GPYRO
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

!Overwrite some Gpyro variables with FDS equivalents
GPG%P0     = P_INF
GPG%RHOINF = RHOA
GPG%GZ     = 0D0 !-GVEC(3)
GPG%GX     = 0D0 !GVEC(1)
...
DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
   IF (USE_MPI .AND. MYID .NE. GPG%PROCESS_GPYRO(IGPYROMESH)) CYCLE
   CALL ALLOCATE_GPYRO(IGPYROMESH)  
   CALL INIT_GPYRO_VARS(1,IGPYROMESH)
ENDDO

!WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 03' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

!Now consolidate ngpyro_cells_needing_bcs with master:
IF (USE_MPI) THEN
   IF (MYID .EQ. 0) THEN
      IF (.NOT. ALLOCATED(NGPYRO_CELLS_NEEDING_BCS)) ALLOCATE(NGPYRO_CELLS_NEEDING_BCS(0:GPG%NUM_GPYRO_MESHES))
      DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
         IFROM = GPG%PROCESS_GPYRO(IGPYROMESH)
         ITAG  = IGPYROMESH
         CALL MPI_RECV(NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), 1, MPI_INTEGER, IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
      ENDDO
   ELSE
      DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
         IF (MYID .NE. GPG%PROCESS_GPYRO(IGPYROMESH)) CYCLE
         CALL MPI_SEND(NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), 1, MPI_INTEGER, 0, IGPYROMESH, MPI_COMM_WORLD, IERR)
      ENDDO
   ENDIF
ENDIF !USE_MPI
!WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 04' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

!And send it from master back to other nodes:
IF (MYID .EQ. 0) NGPYRO_CELLS_NEEDING_BCS(0) = SUM(NGPYRO_CELLS_NEEDING_BCS(1:GPG%NUM_GPYRO_MESHES))

IF (USE_MPI) THEN
   ISIZE = GPG%NUM_GPYRO_MESHES + 1
   IF (MYID .EQ. 0) THEN
      DO IDEST = 1, N_MPI_PROCESSES - 1
         CALL MPI_SEND(NGPYRO_CELLS_NEEDING_BCS(0), ISIZE, MPI_INTEGER, IDEST, IDEST, MPI_COMM_WORLD, IERR)
      ENDDO
   ELSE
      IF (.NOT. ALLOCATED(NGPYRO_CELLS_NEEDING_BCS)) ALLOCATE(NGPYRO_CELLS_NEEDING_BCS(0:GPG%NUM_GPYRO_MESHES))
      CALL MPI_RECV(NGPYRO_CELLS_NEEDING_BCS(0), ISIZE, MPI_INTEGER, 0, MYID, MPI_COMM_WORLD, ISTATUS, IERR)
   ENDIF
ENDIF !USE_MPI

!WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 05' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
WRITE(*,*) 'MYID ', MYID, 'NGPYRO_CELLS_NEEDING_BCS(0): ', NGPYRO_CELLS_NEEDING_BCS(0)

CALL ALLOCATE_GPYRO2FDS(GPG%NUM_GPYRO_MESHES, 0, NGPYRO_CELLS_NEEDING_BCS(0))

!Now consolidate coupling info with master:
IF (.NOT. ALLOCATED(GPYRO2FDS)) ALLOCATE (GPYRO2FDS(0:GPG%NUM_GPYRO_MESHES))

ALLOCATE (GPYRO2FDS(0)%IMESH_GPYRO(1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IMESH_GPYRO(:) = 0
ALLOCATE (GPYRO2FDS(0)%IZ_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IZ_GPYRO   (:) = 0
ALLOCATE (GPYRO2FDS(0)%IX_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IX_GPYRO   (:) = 0
ALLOCATE (GPYRO2FDS(0)%IY_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IY_GPYRO   (:) = 0
ALLOCATE (GPYRO2FDS(0)%IOR_GPYRO  (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IOR_GPYRO  (:) = 0
ALLOCATE (GPYRO2FDS(0)%IOR_FDS    (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IOR_FDS    (:) = 0
ALLOCATE (GPYRO2FDS(0)%IMESH_FDS  (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IMESH_FDS  (:) = 0
ALLOCATE (GPYRO2FDS(0)%I_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%I_FDS      (:) = 0
ALLOCATE (GPYRO2FDS(0)%J_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%J_FDS      (:) = 0
ALLOCATE (GPYRO2FDS(0)%K_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%K_FDS      (:) = 0
ALLOCATE (GPYRO2FDS(0)%IW_FDS     (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%IW_FDS     (:) = 0
ALLOCATE (GPYRO2FDS(0)%RATIO      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%RATIO      (:) = 0
ALLOCATE (GPYRO2FDS(0)%DIFF       (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%DIFF       (:) = 9D9
ALLOCATE (GPYRO2FDS(0)%Z_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%Z_GPYRO    (:) = 0D0
ALLOCATE (GPYRO2FDS(0)%X_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%X_GPYRO    (:) = 0D0
ALLOCATE (GPYRO2FDS(0)%Y_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%Y_GPYRO    (:) = 0D0
ALLOCATE (GPYRO2FDS(0)%X_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%X_FDS      (:) = 0D0
ALLOCATE (GPYRO2FDS(0)%Y_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%Y_FDS      (:) = 0D0
ALLOCATE (GPYRO2FDS(0)%Z_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); GPYRO2FDS(0)%Z_FDS      (:) = 0D0

IF (USE_MPI) THEN
   IF (MYID .EQ. 0) THEN
   
!      WRITE(*,*) 'MYID 0 GOING TO RECEIVE INFO FROM SLAVE PROCESSES'

      ISTART = 1
      DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
         IFROM = GPG%PROCESS_GPYRO(IGPYROMESH)
         IF (IGPYROMESH .NE. 1) ISTART = ISTART + NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH-1)
         ITAG = 1; CALL MPI_RECV(GPYRO2FDS(0)%IMESH_GPYRO(ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 2; CALL MPI_RECV(GPYRO2FDS(0)%IZ_GPYRO   (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 3; CALL MPI_RECV(GPYRO2FDS(0)%IX_GPYRO   (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 4; CALL MPI_RECV(GPYRO2FDS(0)%IY_GPYRO   (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 5; CALL MPI_RECV(GPYRO2FDS(0)%IOR_GPYRO  (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 6; CALL MPI_RECV(GPYRO2FDS(0)%Z_GPYRO    (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 7; CALL MPI_RECV(GPYRO2FDS(0)%X_GPYRO    (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
         ITAG = 8; CALL MPI_RECV(GPYRO2FDS(0)%Y_GPYRO    (ISTART), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, IFROM, ITAG, MPI_COMM_WORLD, ISTATUS, IERR)
      ENDDO

   ELSE
!      WRITE(*,*) 'MYID ', MYID, 'GOING TO SEND INFO TO MASTER PROCESS'

      DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
         IF (MYID .NE. GPG%PROCESS_GPYRO(IGPYROMESH)) CYCLE
         ITAG = 1; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%IMESH_GPYRO(1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 2; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%IZ_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 3; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%IX_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 4; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%IY_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 5; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%IOR_GPYRO  (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_INTEGER         , 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 6; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%Z_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 7; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%X_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, 0, ITAG, MPI_COMM_WORLD, IERR)
         ITAG = 8; CALL MPI_SEND(GPYRO2FDS(IGPYROMESH)%Y_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH), MPI_DOUBLE_PRECISION, 0, ITAG, MPI_COMM_WORLD, IERR)
      ENDDO
   ENDIF
ELSE !USE MPI .EQ. .FALSE.
   ISTART = 1
   DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
      IF (IGPYROMESH .NE. 1) ISTART = ISTART + NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH-1)
      ISTOP = ISTART + NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH) - 1
      GPYRO2FDS(0)%IMESH_GPYRO(ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%IMESH_GPYRO(1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%IZ_GPYRO   (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%IZ_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%IX_GPYRO   (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%IX_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%IY_GPYRO   (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%IY_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%IOR_GPYRO  (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%IOR_GPYRO  (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%Z_GPYRO    (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%Z_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%X_GPYRO    (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%X_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
      GPYRO2FDS(0)%Y_GPYRO    (ISTART:ISTOP) = GPYRO2FDS(IGPYROMESH)%Y_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(IGPYROMESH)-1) 
   ENDDO
ENDIF !USE MPI

WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 06' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

!Now send coupling info from master to other nodes:
IF (USE_MPI) THEN
    CALL MPI_BCAST(GPYRO2FDS(0)%IMESH_GPYRO(1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_INTEGER         , 0, MPI_COMM_WORLD, IERR)
    CALL MPI_BCAST(GPYRO2FDS(0)%IZ_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_INTEGER         , 0, MPI_COMM_WORLD, IERR)
    CALL MPI_BCAST(GPYRO2FDS(0)%IX_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_INTEGER         , 0, MPI_COMM_WORLD, IERR)
    CALL MPI_BCAST(GPYRO2FDS(0)%IY_GPYRO   (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_INTEGER         , 0, MPI_COMM_WORLD, IERR)
    CALL MPI_BCAST(GPYRO2FDS(0)%IOR_GPYRO  (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_INTEGER         , 0, MPI_COMM_WORLD, IERR) 
    CALL MPI_BCAST(GPYRO2FDS(0)%Z_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR) 
    CALL MPI_BCAST(GPYRO2FDS(0)%X_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR) 
    CALL MPI_BCAST(GPYRO2FDS(0)%Y_GPYRO    (1), NGPYRO_CELLS_NEEDING_BCS(0), MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR) 
ENDIF

WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 07' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
CALL MAP_GPYRO_TO_FDS

WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 08' 
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

IF (USE_MPI) THEN
   IF (MYID .EQ. 0) THEN

      WRITE(*,*) 'MYID ', MYID, 'ALLOCATING MAP'
      ALLOCATE (MAPPING(1))
      ALLOCATE (MAPPING(1)%IMESH_GPYRO(1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IMESH_GPYRO(:) = 0
      ALLOCATE (MAPPING(1)%IZ_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IZ_GPYRO   (:) = 0
      ALLOCATE (MAPPING(1)%IX_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IX_GPYRO   (:) = 0
      ALLOCATE (MAPPING(1)%IY_GPYRO   (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IY_GPYRO   (:) = 0
      ALLOCATE (MAPPING(1)%IOR_GPYRO  (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IOR_GPYRO  (:) = 0
      ALLOCATE (MAPPING(1)%IOR_FDS    (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IOR_FDS    (:) = 0
      ALLOCATE (MAPPING(1)%IMESH_FDS  (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IMESH_FDS  (:) = 0
      ALLOCATE (MAPPING(1)%I_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%I_FDS      (:) = 0
      ALLOCATE (MAPPING(1)%J_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%J_FDS      (:) = 0
      ALLOCATE (MAPPING(1)%K_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%K_FDS      (:) = 0
      ALLOCATE (MAPPING(1)%IW_FDS     (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%IW_FDS     (:) = 0
      ALLOCATE (MAPPING(1)%RATIO      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%RATIO      (:) = 0      
      ALLOCATE (MAPPING(1)%DIFF       (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%DIFF       (:) = 9D9
      ALLOCATE (MAPPING(1)%XDIFF      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%XDIFF      (:) = 9D9
      ALLOCATE (MAPPING(1)%YDIFF      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%YDIFF      (:) = 9D9
      ALLOCATE (MAPPING(1)%ZDIFF      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%ZDIFF      (:) = 9D9
      ALLOCATE (MAPPING(1)%Z_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%Z_GPYRO    (:) = 0D0
      ALLOCATE (MAPPING(1)%X_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%X_GPYRO    (:) = 0D0
      ALLOCATE (MAPPING(1)%Y_GPYRO    (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%Y_GPYRO    (:) = 0D0
      ALLOCATE (MAPPING(1)%X_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%X_FDS      (:) = 0D0
      ALLOCATE (MAPPING(1)%Y_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%Y_FDS      (:) = 0D0
      ALLOCATE (MAPPING(1)%Z_FDS      (1:NGPYRO_CELLS_NEEDING_BCS(0))); MAPPING(1)%Z_FDS      (:) = 0D0

      WRITE(*,*) 'MYID ', MYID, 'RECEIVING MAP'
      DO IRECV = 1, N_MPI_PROCESSES - 1
         CALL RECV_MAP(IRECV,MAPPING(1),NGPYRO_CELLS_NEEDING_BCS(0))
         DO ICOUNT = 1, NGPYRO_CELLS_NEEDING_BCS(0)
            IF (MAPPING(1)%DIFF(ICOUNT) .LT. GPYRO2FDS(0)%DIFF(ICOUNT)) THEN
               GPYRO2FDS(0)%IMESH_FDS(ICOUNT) = MAPPING(1)%IMESH_FDS(ICOUNT)
               GPYRO2FDS(0)%IW_FDS   (ICOUNT) = MAPPING(1)%IW_FDS   (ICOUNT)
               GPYRO2FDS(0)%RATIO    (ICOUNT) = MAPPING(1)%RATIO    (ICOUNT)               
               GPYRO2FDS(0)%I_FDS    (ICOUNT) = MAPPING(1)%I_FDS    (ICOUNT)
               GPYRO2FDS(0)%J_FDS    (ICOUNT) = MAPPING(1)%J_FDS    (ICOUNT)
               GPYRO2FDS(0)%K_FDS    (ICOUNT) = MAPPING(1)%K_FDS    (ICOUNT)
               GPYRO2FDS(0)%IOR_FDS  (ICOUNT) = MAPPING(1)%IOR_FDS  (ICOUNT)
               GPYRO2FDS(0)%DIFF     (ICOUNT) = MAPPING(1)%DIFF     (ICOUNT)
               GPYRO2FDS(0)%XDIFF    (ICOUNT) = MAPPING(1)%XDIFF    (ICOUNT)
               GPYRO2FDS(0)%YDIFF    (ICOUNT) = MAPPING(1)%YDIFF    (ICOUNT)
               GPYRO2FDS(0)%ZDIFF    (ICOUNT) = MAPPING(1)%ZDIFF    (ICOUNT)               
               GPYRO2FDS(0)%X_FDS    (ICOUNT) = MAPPING(1)%X_FDS    (ICOUNT)
               GPYRO2FDS(0)%Y_FDS    (ICOUNT) = MAPPING(1)%Y_FDS    (ICOUNT)
               GPYRO2FDS(0)%Z_FDS    (ICOUNT) = MAPPING(1)%Z_FDS    (ICOUNT)
            ENDIF
         ENDDO !ICOUNT
      ENDDO !IRECV
      DEALLOCATE(MAPPING)
   ELSE
      WRITE(*,*) 'MYID ', MYID, 'SENDING MAP'
      CALL SEND_MAP(0,GPYRO2FDS(0),NGPYRO_CELLS_NEEDING_BCS(0))
   ENDIF !MYID .EQ. 0

   WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 09'
   CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

!Address possibility of multiple couplings:
   WRITE(*,*) 'MYID ', MYID, 'INITIAL CHECK FOR MULTIPLE COUPLINGS'
   DO I = 1, NGPYRO_CELLS_NEEDING_BCS(0)
      ICOUNT = 0
      IF (GPYRO2FDS(0)%IMESH_FDS(I) .LE. 0) CYCLE      
      DO J = 1, NGPYRO_CELLS_NEEDING_BCS(0)
         IF (GPYRO2FDS(0)%IMESH_FDS(J) .NE. GPYRO2FDS(0)%IMESH_FDS(I)) CYCLE 
         IF (GPYRO2FDS(0)%IW_FDS   (J) .NE. GPYRO2FDS(0)%IW_FDS   (I)) CYCLE 
         ICOUNT = ICOUNT + 1
      ENDDO !J
      GPYRO2FDS(0)%RATIO(I) = ICOUNT
   ENDDO !I
   WRITE(*,*) 'MYID ', MYID, 'DONE WITH INITIAL CHECK FOR MULTIPLE COUPLINGS'

   CALL SEND_RECV_MAP_BCAST(GPYRO2FDS(0),NGPYRO_CELLS_NEEDING_BCS(0))
ELSE
!Address possibility of multiple couplings:
   DO I = 1, NGPYRO_CELLS_NEEDING_BCS(0)
      ICOUNT = 0
...
WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 10'
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

CALL MAP_GPYRO_TO_GPYRO
WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 11'
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)

IF (MYID .EQ. 0) THEN

   LUMAP = 998 
   OPEN(UNIT=LUMAP,FILE='0.csv',FORM='FORMATTED',STATUS='REPLACE')

   WRITE(LUMAP,2000) 'ICOUNT,IMESH_GPYRO,IX_GPYRO,IY_GPYRO,IZ_GPYRO,X_GPYRO,Y_GPYRO,Z_GPYRO,IOR_GPYRO,IW_FDS,RATIO,IMESH_FDS,I_FDS,J_FDS,K_FDS,X_FDS,Y_FDS,Z_FDS,IOR_FDS,DIFF,XDIFF,YDIFF,ZDIFF'

   DO ICOUNT = 1, NGPYRO_CELLS_NEEDING_BCS(0)
      WRITE(LUMAP,3000) ICOUNT,&
      GPYRO2FDS(0)%IMESH_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IX_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IY_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IZ_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%X_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%Y_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%Z_GPYRO(ICOUNT), &      
      GPYRO2FDS(0)%IOR_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IW_FDS(ICOUNT), &
      GPYRO2FDS(0)%RATIO(ICOUNT), &
      GPYRO2FDS(0)%IMESH_FDS(ICOUNT), & 
      GPYRO2FDS(0)%I_FDS(ICOUNT), &
      GPYRO2FDS(0)%J_FDS(ICOUNT), &
      GPYRO2FDS(0)%K_FDS(ICOUNT), &
      GPYRO2FDS(0)%X_FDS(ICOUNT), &
      GPYRO2FDS(0)%Y_FDS(ICOUNT), &
      GPYRO2FDS(0)%Z_FDS(ICOUNT), &
      GPYRO2FDS(0)%IOR_FDS(ICOUNT), &
      GPYRO2FDS(0)%DIFF(ICOUNT), &
      GPYRO2FDS(0)%XDIFF(ICOUNT), &
      GPYRO2FDS(0)%YDIFF(ICOUNT), &
      GPYRO2FDS(0)%ZDIFF(ICOUNT)
   ENDDO

   CLOSE(LUMAP)

   OPEN(UNIT=LUMAP,FILE='bad_couplings.csv',FORM='FORMATTED',STATUS='REPLACE')

   WRITE(LUMAP,2000) 'ICOUNT,IMESH_GPYRO,IX_GPYRO,IY_GPYRO,IZ_GPYRO,X_GPYRO,Y_GPYRO,Z_GPYRO,IOR_GPYRO,IW_FDS,RATIO,IMESH_FDS,I_FDS,J_FDS,K_FDS,X_FDS,Y_FDS,Z_FDS,IOR_FDS,DIFF,XDIFF,YDIFF,ZDIFF'

   DO ICOUNT = 1, NGPYRO_CELLS_NEEDING_BCS(0)
      IF (GPYRO2FDS(0)%DIFF(ICOUNT) .LT. 1E6) CYCLE
      WRITE(LUMAP,3000) ICOUNT,&
      GPYRO2FDS(0)%IMESH_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IX_GPYRO(ICOUNT), &
      GPYRO2FDS(0)%IY_GPYRO(ICOUNT), &
...
2000 FORMAT(A)
3000 FORMAT(5(I6,','),3(F10.7,','),7(I6,','),3(F10.7,','),1(I6,','),4(E13.7,','))

WRITE(*,*) 'MYID ', MYID, 'MPI_BARRIER 12'
CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
...
MAIN_LOOP: DO  
!Begin CWL addition:
   IF (T(1) .GT. TSTART_SKIP_FDS_TIMESEPS .AND. SKIP_FDS_TIMESTEPS) THEN
      DO ISKIP = 1, NSKIP_FDS_TIMESTEPS - 1
         DO NM=1,NMESHES
            T(NM) = T(NM) + MESHES(NM)%DT                                  
         ENDDO
      ENDDO
   ENDIF
!   WRITE(*,*) 'At top of main timestepping loop'
!End CWL addition
   ...
   IF (N_MPI_PROCESSES>1 .AND. MYID .LE. NMESHES-1) THEN !CWL:  Added .AND. MYID .LE. NMESHES-1
      CALL MPI_ALLGATHERV(MPI_IN_PLACE,COUNTS(MYID),MPI_DOUBLE_PRECISION,T,COUNTS,DISPLS,MPI_DOUBLE_PRECISION,MPI_COMM_WORLD,IERR)
...
!Loop over interfaces and determine T and k from various meshes
   DO ICOUNT = 1, G2GIFACE%NINTERFACES
      ...
      IF (.NOT. USE_MPI .OR. MYID .EQ. GPG%PROCESS_GPYRO(IMESH1)) THEN
         ...
         G2GIFACE%TMP1(ICOUNT) = GPM(IMESH1)%TPN(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 1) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KX(IZ,IX,IY) + GPM(IMESH1)%KRX(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 2) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KY(IZ,IX,IY) + GPM(IMESH1)%KRY(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 3) G2GIFACE%K1(ICOUNT) = GPM(IMESH1)%KZ(IZ,IX,IY) + GPM(IMESH1)%KRZ(IZ,IX,IY)

         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 1) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%DLTXN(IZ,IX,IY) 
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 2) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%DLTYN(IZ,IX,IY)  
         IF (ABS(G2GIFACE%IOR1(ICOUNT)) .EQ. 3) G2GIFACE%DLT1(ICOUNT) = GPM(IMESH1)%DLTZN(IZ,IX,IY)  
      ENDIF
      ...
      IF (.NOT. USE_MPI .OR. MYID .EQ. GPG%PROCESS_GPYRO(IMESH2)) THEN
         ...
         G2GIFACE%TMP2(ICOUNT) = GPM(IMESH2)%TPN(IZ,IX,IY)

         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 1) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KX(IZ,IX,IY) + GPM(IMESH2)%KRX(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 2) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KY(IZ,IX,IY) + GPM(IMESH2)%KRY(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 3) G2GIFACE%K2(ICOUNT) = GPM(IMESH2)%KZ(IZ,IX,IY) + GPM(IMESH2)%KRZ(IZ,IX,IY)

         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 1) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%DLTXN(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 2) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%DLTYN(IZ,IX,IY)
         IF (ABS(G2GIFACE%IOR2(ICOUNT)) .EQ. 3) G2GIFACE%DLT2(ICOUNT) = GPM(IMESH2)%DLTZN(IZ,IX,IY)
      ENDIF
...
!Since it's not a lot of traffic, have all slave nodes send their ifaces to master node

   IF (USE_MPI .AND. G2GIFACE%NINTERFACES .GT. 0) THEN
      
      IF (MYID .EQ. 0) THEN

         DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
            CALL RECV_IFACE(GPG%PROCESS_GPYRO(IGPYROMESH),G2GIFACE%NINTERFACES,IGPYROMESH,IFACETEMP)

            DO ICOUNT = 1, G2GIFACE%NINTERFACES
               IF (IFACETEMP%IMESH1(ICOUNT) .EQ. IGPYROMESH) THEN
                  G2GIFACE%TMP1(ICOUNT) = IFACETEMP%TMP1(ICOUNT)
                  G2GIFACE%K1  (ICOUNT) = IFACETEMP%K1  (ICOUNT)
                  G2GIFACE%DLT1(ICOUNT) = IFACETEMP%DLT1(ICOUNT)
               ENDIF
               IF (IFACETEMP%IMESH2(ICOUNT) .EQ. IGPYROMESH) THEN
                  G2GIFACE%TMP2(ICOUNT) = IFACETEMP%TMP1(ICOUNT)
                  G2GIFACE%K2  (ICOUNT) = IFACETEMP%K1  (ICOUNT)
                  G2GIFACE%DLT2(ICOUNT) = IFACETEMP%DLT2(ICOUNT)
               ENDIF
            ENDDO !ICOUNT
      ...
      ELSE !MYID .NE. 0
         ...
         DO IGPYROMESH = 1, GPG%NUM_GPYRO_MESHES
            IF (MYID .NE. GPG%PROCESS_GPYRO(IGPYROMESH)) CYCLE
            CALL SEND_IFACE(0,G2GIFACE%NINTERFACES,IGPYROMESH,G2GIFACE)
         ENDDO
      
      ENDIF !MYID .EQ. 0
   
   ENDIF !USE_MPI .AND. G2GIFACE%NINTERFACES .GT. 0
   
! Now calculate heat flux
   DO ICOUNT = 1, G2GIFACE%NINTERFACES
     DELTAX = 0.5*(G2GIFACE%DLT1(ICOUNT)+G2GIFACE%DLT2(ICOUNT))
     DELTAT = G2GIFACE%TMP1(ICOUNT) - G2GIFACE%TMP2(ICOUNT)
     KINTERFACE = 2D0*G2GIFACE%K1(ICOUNT)*G2GIFACE%K2(ICOUNT) / (G2GIFACE%K1(ICOUNT) + G2GIFACE%K2(ICOUNT))
     G2GIFACE%Q(ICOUNT) = -KINTERFACE * DELTAT / DELTAX
   ENDDO
...
! CWL: This is where call to pyrolysis is made:  
!WRITE(*,*) 'MYID ', MYID, 'BEFORE COMPUTE_WALL_BC_2A' !CWLDELETEME
...
!WRITE(*,*) 'MYID ', MYID, ' BEFORE MPI_BARRIER' !CWLDELETEME
   CALL MPI_BARRIER(MPI_COMM_WORLD, IERR) !CWL
!WRITE(*,*) 'MYID ', MYID, ' AFTER MPI_BARRIER' !CWLDELETEME
   ...
   RECEIVING_MESH_LOOP: DO NOM=1,NMESHES
      ...
      IF (CODE==1 .AND. M3%NIC_S>0) THEN
         ...
         ELSE
            ...
            M2%ZZS(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)= &
      ...
      IF (CODE==4 .AND. M3%NIC_S>0) THEN
         ...
         ELSE
            ...
            M2%ZZ(IMIN:IMAX,JMIN:JMAX,KMIN:KMAX,1:N_TRACKED_SPECIES)= &
...
! *****************************************************************************
SUBROUTINE SEND_MAP(IDEST,MAPTOSEND,NMAP)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: IDEST,NMAP
TYPE(GPYRO_TO_FDS_COUPLING_TYPE), INTENT(IN) :: MAPTOSEND
INTEGER , DIMENSION(1:12,1:NMAP) :: ISENDBUF
REAL(EB), DIMENSION(1:10,1:NMAP) :: RSENDBUF
INTEGER :: IERR,ISIZEINT,ISIZEDOUBLE

ISIZEINT   =12*NMAP
ISIZEDOUBLE=10*NMAP

DO ICOUNT = 1, NMAP
   ISENDBUF( 1,ICOUNT) = MAPTOSEND%IMESH_GPYRO(ICOUNT)
   ISENDBUF( 2,ICOUNT) = MAPTOSEND%IZ_GPYRO(ICOUNT)
   ISENDBUF( 3,ICOUNT) = MAPTOSEND%IX_GPYRO(ICOUNT)
   ISENDBUF( 4,ICOUNT) = MAPTOSEND%IY_GPYRO(ICOUNT)
   ISENDBUF( 5,ICOUNT) = MAPTOSEND%IOR_GPYRO(ICOUNT)
   ISENDBUF( 6,ICOUNT) = MAPTOSEND%IOR_FDS(ICOUNT)
   ISENDBUF( 7,ICOUNT) = MAPTOSEND%IMESH_FDS(ICOUNT)
   ISENDBUF( 8,ICOUNT) = MAPTOSEND%I_FDS(ICOUNT)
   ISENDBUF( 9,ICOUNT) = MAPTOSEND%J_FDS(ICOUNT)
   ISENDBUF(10,ICOUNT) = MAPTOSEND%K_FDS(ICOUNT)
   ISENDBUF(11,ICOUNT) = MAPTOSEND%IW_FDS(ICOUNT)
   ISENDBUF(12,ICOUNT) = MAPTOSEND%RATIO(ICOUNT)   
   RSENDBUF( 1,ICOUNT) = MAPTOSEND%DIFF(ICOUNT)
   RSENDBUF( 2,ICOUNT) = MAPTOSEND%Z_GPYRO(ICOUNT)
   RSENDBUF( 3,ICOUNT) = MAPTOSEND%X_GPYRO(ICOUNT)
   RSENDBUF( 4,ICOUNT) = MAPTOSEND%Y_GPYRO(ICOUNT)
   RSENDBUF( 5,ICOUNT) = MAPTOSEND%X_FDS(ICOUNT)
   RSENDBUF( 6,ICOUNT) = MAPTOSEND%Y_FDS(ICOUNT)
   RSENDBUF( 7,ICOUNT) = MAPTOSEND%Z_FDS(ICOUNT)
   RSENDBUF( 8,ICOUNT) = MAPTOSEND%XDIFF(ICOUNT)
   RSENDBUF( 9,ICOUNT) = MAPTOSEND%YDIFF(ICOUNT)
   RSENDBUF(10,ICOUNT) = MAPTOSEND%ZDIFF(ICOUNT)   
ENDDO

CALL MPI_SEND(ISENDBUF(:,:), ISIZEINT   , MPI_INTEGER         , IDEST, 1, MPI_COMM_WORLD, IERR)
CALL MPI_SEND(RSENDBUF(:,:), ISIZEDOUBLE, MPI_DOUBLE_PRECISION, IDEST, 2, MPI_COMM_WORLD, IERR)

! *****************************************************************************
END SUBROUTINE SEND_MAP
! *****************************************************************************

! *****************************************************************************
SUBROUTINE RECV_MAP(IFROM,MAPTORECV,NMAP)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: IFROM,NMAP
TYPE(GPYRO_TO_FDS_COUPLING_TYPE), INTENT(OUT) :: MAPTORECV
REAL(EB), DIMENSION(1:10,1:NMAP) :: RRECVBUF
INTEGER , DIMENSION(1:12,1:NMAP) :: IRECVBUF
INTEGER :: ISTATUS(MPI_STATUS_SIZE),IERR,ISIZEINT,ISIZEDOUBLE

ISIZEINT   =12*NMAP
ISIZEDOUBLE=10*NMAP

CALL MPI_RECV(IRECVBUF(:,:), ISIZEINT   , MPI_INTEGER         , IFROM, 1, MPI_COMM_WORLD, ISTATUS, IERR)
CALL MPI_RECV(RRECVBUF(:,:), ISIZEDOUBLE, MPI_DOUBLE_PRECISION, IFROM, 2, MPI_COMM_WORLD, ISTATUS, IERR)

DO ICOUNT = 1, NMAP
    MAPTORECV%IMESH_GPYRO(ICOUNT) = IRECVBUF( 1,ICOUNT)  
    MAPTORECV%IZ_GPYRO(ICOUNT)    = IRECVBUF( 2,ICOUNT)  
    MAPTORECV%IX_GPYRO(ICOUNT)    = IRECVBUF( 3,ICOUNT)  
    MAPTORECV%IY_GPYRO(ICOUNT)    = IRECVBUF( 4,ICOUNT)  
    MAPTORECV%IOR_GPYRO(ICOUNT)   = IRECVBUF( 5,ICOUNT)  
    MAPTORECV%IOR_FDS(ICOUNT)     = IRECVBUF( 6,ICOUNT)  
    MAPTORECV%IMESH_FDS(ICOUNT)   = IRECVBUF( 7,ICOUNT)  
    MAPTORECV%I_FDS(ICOUNT)       = IRECVBUF( 8,ICOUNT)  
    MAPTORECV%J_FDS(ICOUNT)       = IRECVBUF( 9,ICOUNT)  
    MAPTORECV%K_FDS(ICOUNT)       = IRECVBUF(10,ICOUNT)  
    MAPTORECV%IW_FDS(ICOUNT)      = IRECVBUF(11,ICOUNT)  
    MAPTORECV%RATIO(ICOUNT)       = IRECVBUF(12,ICOUNT)  
    MAPTORECV%DIFF(ICOUNT)        = RRECVBUF( 1, ICOUNT) 
    MAPTORECV%Z_GPYRO(ICOUNT)     = RRECVBUF( 2, ICOUNT) 
    MAPTORECV%X_GPYRO(ICOUNT)     = RRECVBUF( 3, ICOUNT) 
    MAPTORECV%Y_GPYRO(ICOUNT)     = RRECVBUF( 4, ICOUNT) 
    MAPTORECV%X_FDS(ICOUNT)       = RRECVBUF( 5, ICOUNT) 
    MAPTORECV%Y_FDS(ICOUNT)       = RRECVBUF( 6, ICOUNT) 
    MAPTORECV%Z_FDS(ICOUNT)       = RRECVBUF( 7, ICOUNT) 
    MAPTORECV%XDIFF(ICOUNT)       = RRECVBUF( 8, ICOUNT) 
    MAPTORECV%YDIFF(ICOUNT)       = RRECVBUF( 9, ICOUNT) 
    MAPTORECV%ZDIFF(ICOUNT)       = RRECVBUF(10, ICOUNT)     
ENDDO

! *****************************************************************************
END SUBROUTINE RECV_MAP
! *****************************************************************************

! *****************************************************************************
SUBROUTINE SEND_RECV_MAP_BCAST(GMAP,NMAP)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: NMAP
TYPE(GPYRO_TO_FDS_COUPLING_TYPE), INTENT(IN) :: GMAP
INTEGER , DIMENSION(1:12,1:NMAP) :: IBUF
REAL(EB), DIMENSION(1:10,1:NMAP) :: RBUF
INTEGER :: IERR,ISIZEINT,ISIZEDOUBLE
...
IF (MYID .EQ. 0) THEN
   DO ICOUNT = 1, NMAP
      IBUF( 1,ICOUNT) = GMAP%IMESH_GPYRO(ICOUNT)
      IBUF( 2,ICOUNT) = GMAP%IZ_GPYRO(ICOUNT)
      IBUF( 3,ICOUNT) = GMAP%IX_GPYRO(ICOUNT)
      IBUF( 4,ICOUNT) = GMAP%IY_GPYRO(ICOUNT)
      IBUF( 5,ICOUNT) = GMAP%IOR_GPYRO(ICOUNT)
      IBUF( 6,ICOUNT) = GMAP%IOR_FDS(ICOUNT)
      IBUF( 7,ICOUNT) = GMAP%IMESH_FDS(ICOUNT)
      IBUF( 8,ICOUNT) = GMAP%I_FDS(ICOUNT)
      IBUF( 9,ICOUNT) = GMAP%J_FDS(ICOUNT)
      IBUF(10,ICOUNT) = GMAP%K_FDS(ICOUNT)
      IBUF(11,ICOUNT) = GMAP%IW_FDS(ICOUNT)
      IBUF(12,ICOUNT) = GMAP%RATIO(ICOUNT)   
      RBUF( 1,ICOUNT) = GMAP%DIFF(ICOUNT)
      RBUF( 2,ICOUNT) = GMAP%Z_GPYRO(ICOUNT)
      RBUF( 3,ICOUNT) = GMAP%X_GPYRO(ICOUNT)
      RBUF( 4,ICOUNT) = GMAP%Y_GPYRO(ICOUNT)
      RBUF( 5,ICOUNT) = GMAP%X_FDS(ICOUNT)
      RBUF( 6,ICOUNT) = GMAP%Y_FDS(ICOUNT)
      RBUF( 7,ICOUNT) = GMAP%Z_FDS(ICOUNT)
      RBUF( 8,ICOUNT) = GMAP%XDIFF(ICOUNT)
      RBUF( 9,ICOUNT) = GMAP%YDIFF(ICOUNT)
      RBUF(10,ICOUNT) = GMAP%ZDIFF(ICOUNT)   
   ENDDO
ENDIF

CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
CALL MPI_BCAST(IBUF, ISIZEINT   , MPI_INTEGER,          0, MPI_COMM_WORLD, IERR)

CALL MPI_BARRIER(MPI_COMM_WORLD, IERR)
CALL MPI_BCAST(RBUF, ISIZEDOUBLE, MPI_DOUBLE_PRECISION, 0, MPI_COMM_WORLD, IERR)

IF (MYID .NE. 0) THEN
   DO ICOUNT = 1, NMAP
       GMAP%IMESH_GPYRO(ICOUNT) = IBUF( 1,ICOUNT)  
       GMAP%IZ_GPYRO(ICOUNT)    = IBUF( 2,ICOUNT)  
       GMAP%IX_GPYRO(ICOUNT)    = IBUF( 3,ICOUNT)  
       GMAP%IY_GPYRO(ICOUNT)    = IBUF( 4,ICOUNT)  
       GMAP%IOR_GPYRO(ICOUNT)   = IBUF( 5,ICOUNT)  
       GMAP%IOR_FDS(ICOUNT)     = IBUF( 6,ICOUNT)  
       GMAP%IMESH_FDS(ICOUNT)   = IBUF( 7,ICOUNT)  
       GMAP%I_FDS(ICOUNT)       = IBUF( 8,ICOUNT)  
       GMAP%J_FDS(ICOUNT)       = IBUF( 9,ICOUNT)  
       GMAP%K_FDS(ICOUNT)       = IBUF(10,ICOUNT)  
       GMAP%IW_FDS(ICOUNT)      = IBUF(11,ICOUNT)  
       GMAP%RATIO(ICOUNT)       = IBUF(12,ICOUNT)  
       GMAP%DIFF(ICOUNT)        = RBUF( 1, ICOUNT) 
       GMAP%Z_GPYRO(ICOUNT)     = RBUF( 2, ICOUNT) 
       GMAP%X_GPYRO(ICOUNT)     = RBUF( 3, ICOUNT) 
       GMAP%Y_GPYRO(ICOUNT)     = RBUF( 4, ICOUNT) 
       GMAP%X_FDS(ICOUNT)       = RBUF( 5, ICOUNT) 
       GMAP%Y_FDS(ICOUNT)       = RBUF( 6, ICOUNT) 
       GMAP%Z_FDS(ICOUNT)       = RBUF( 7, ICOUNT) 
       GMAP%XDIFF(ICOUNT)       = RBUF( 8, ICOUNT) 
       GMAP%YDIFF(ICOUNT)       = RBUF( 9, ICOUNT) 
       GMAP%ZDIFF(ICOUNT)       = RBUF(10, ICOUNT)     
   ENDDO
ENDIF

! *****************************************************************************
END SUBROUTINE SEND_RECV_MAP_BCAST
! *****************************************************************************

! *****************************************************************************
SUBROUTINE SEND_IFACE(IDEST,NIFACE,IMESH,IFACE)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: IDEST,NIFACE,IMESH
TYPE(GPYRO_TO_GPYRO_INTERFACE) :: IFACE
REAL(EB), DIMENSION(1:6*NIFACE) :: RSENDBUF
INTEGER :: I,IERR

I = 0
DO ICOUNT = 1, NIFACE  
   I=I+1 ; RSENDBUF(I) = IFACE%TMP1(ICOUNT)
   I=I+1 ; RSENDBUF(I) = IFACE%TMP2(ICOUNT)
   I=I+1 ; RSENDBUF(I) = IFACE%K1  (ICOUNT)
   I=I+1 ; RSENDBUF(I) = IFACE%K2  (ICOUNT)
   I=I+1 ; RSENDBUF(I) = IFACE%DLT1(ICOUNT)
   I=I+1 ; RSENDBUF(I) = IFACE%DLT2(ICOUNT)
ENDDO

CALL MPI_SEND(RSENDBUF(:), 6*NIFACE, MPI_DOUBLE_PRECISION, IDEST, IMESH, MPI_COMM_WORLD, IERR)

! *****************************************************************************
END SUBROUTINE SEND_IFACE
! *****************************************************************************

! *****************************************************************************
SUBROUTINE RECV_IFACE(IFROM,NIFACE,IMESH,IFACE)
! *****************************************************************************

IMPLICIT NONE

INTEGER, INTENT(IN) :: IFROM,NIFACE,IMESH
TYPE(GPYRO_TO_GPYRO_INTERFACE) :: IFACE
REAL(EB), DIMENSION(1:6*NIFACE) :: RRECVBUF
INTEGER :: I,IERR,ISTATUS(MPI_STATUS_SIZE)

CALL MPI_RECV(RRECVBUF(:), 6*NIFACE, MPI_DOUBLE_PRECISION, IFROM, IMESH, MPI_COMM_WORLD, ISTATUS, IERR)

I=0
DO ICOUNT = 1, NIFACE  
   I=I+1;IFACE%TMP1(ICOUNT) = RRECVBUF(I)
   I=I+1;IFACE%TMP2(ICOUNT) = RRECVBUF(I)
   I=I+1;IFACE%K1  (ICOUNT) = RRECVBUF(I) 
   I=I+1;IFACE%K2  (ICOUNT) = RRECVBUF(I) 
   I=I+1;IFACE%DLT1(ICOUNT) = RRECVBUF(I) 
   I=I+1;IFACE%DLT2(ICOUNT) = RRECVBUF(I) 
ENDDO

! *****************************************************************************
END SUBROUTINE RECV_IFACE
! *****************************************************************************
