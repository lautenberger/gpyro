...
CHARACTER(80) :: TCFORM
LOGICAL :: EX,DRY
CHARACTER(255), PARAMETER :: dumpid='$Id: dump.f90 22343 2015-04-11 14:12:43Z mcgratta $'
CHARACTER(255), PARAMETER :: dumprev='$Revision: 22343 $'
CHARACTER(255), PARAMETER :: dumpdate='$Date: 2015-04-11 07:12:43 -0700 (Sat, 11 Apr 2015) $'
   ...
   ! write out slice file info to the .smv file
      ...
      IF (SL%ID/='null') THEN
         IF (.NOT.SL%TERRAIN_SLICE.AND..NOT.SL%CELL_CENTERED) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A,A,A)') 'SLCF',NM,' %',TRIM(SL%ID),TRIM(SLICEPARMS)
         ENDIF
         IF (SL%CELL_CENTERED) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A,A,A)') 'SLCC',NM,' %',TRIM(SL%ID),TRIM(SLICEPARMS)
         ENDIF
      ELSE
         IF (.NOT.SL%TERRAIN_SLICE.AND..NOT.SL%CELL_CENTERED) THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'SLCF',NM,TRIM(SLICEPARMS)
         ENDIF
         IF (SL%CELL_CENTERED)  THEN
            WRITE(M%STRING(M%N_STRINGS),'(A,I6,A)') 'SLCC',NM,TRIM(SLICEPARMS)
         ENDIF
      ENDIF
...
EVAC_ONLY_PROPS: IF (ANY(EVACUATION_GRID)) THEN
   DO N=1,N_EVAC
      WRITE(LU_SMV,'(/A)') 'PROP'
...
WRITE(LU_CORE(NM)) DS
WRITE(LU_CORE(NM)) HS
IF (N_REACTIONS>0) WRITE(LU_CORE(NM)) D_REACTION
...
DO N=1,N_DEVC
   ...
   WRITE(LU_CORE(NM)) DV%RMS_VALUE,DV%RMS_VALUE2,DV%COV_VALUE,DV%AVERAGE_VALUE,DV%AVERAGE_VALUE2,&
                      ...
                      DV%TIME_INTERVAL,DV%CURRENT_STATE,DV%PRIOR_STATE,DV%LP_TAG,DV%PART_INDEX
...
IF (N_REACTIONS>0) READ(LU_RESTART(NM)) D_REACTION
...
DO N=1,N_LAGRANGIAN_CLASSES
   ...
   IF (PARTICLE_STORAGE(N)%N_STORAGE_SLOTS>0) THEN
      ...
      READ(LU_RESTART(NM)) PARTICLE_STORAGE(N)%REALS
      READ(LU_RESTART(NM)) PARTICLE_STORAGE(N)%INTEGERS
      READ(LU_RESTART(NM)) PARTICLE_STORAGE(N)%LOGICALS
      DO NS=1,PARTICLE_STORAGE(N)%N_STORAGE_SLOTS
         IF (PARTICLE_STORAGE(N)%INTEGERS(1,NS)>0) THEN
            CALL ALLOCATE_STORAGE(NM,LAGRANGIAN_PARTICLE_CLASS(N)%SURF_INDEX,LPC_INDEX=N, &
                                  LP_INDEX=PARTICLE_STORAGE(N)%INTEGERS(2,NS),TAG=PARTICLE_STORAGE(N)%INTEGERS(1,NS), &
                                  NEW_TAG=.FALSE.)
         ENDIF
      ENDDO
   ENDIF
...
ISOF_LOOP: DO N=1,N_ISOF
   ...
   DO K=0,KBP1
      DO J=0,JBP1
         DO I=0,IBP1
            QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IS%INDEX,0,IS%Y_INDEX,IS%Z_INDEX,0,IS%VELO_INDEX,0,0,T,NM)
         ENDDO
      ENDDO
   ENDDO
   ...
   DO K=0,KBAR
      DO J=0,JBAR
         DO I=0,IBAR
            QQ(I+1,J+1,K+1,1) = S(I,J,K)*(QUANTITY(I,J,K)*B(I,J,K)+ QUANTITY(I+1,J,K)*B(I+1,J,K)+ &
      ...
      CASE(1)
         DO K=0,KBP1
            DO J=0,JBP1
               DO I=0,IBP1
                  FF(I,J,K) = RHO(I,J,K)*&
                       GAS_PHASE_OUTPUT(I,J,K,SMOKE3D_QUANTITY_INDEX,0,SMOKE3D_Y_INDEX,SMOKE3D_Z_INDEX,0,0,0,0,T,NM)
               ENDDO
            ENDDO
         ENDDO
      CASE(2)
...
INTEGER :: IFRMT,I,J,K,NQT,I1,I2,J1,J2,K1,K2,ITM,ITM1,IQ,IQQ,IND,IND2,II1,II2,JJ1,JJ2,KK1,KK2, &
           IC,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PROP_INDEX
INTEGER :: KTS,NTSL
...
IF (.NOT.PLOT3D) THEN
   DO IQ=1,N_SLCF
      SL => SLICE(IQ)
      IF (OUTPUT_QUANTITY(SL%INDEX)%INTEGRATED_PARTICLES) P_FLUX = .TRUE.
   ENDDO
ELSE
   DO IQ=1,5
      IF (OUTPUT_QUANTITY(PLOT3D_QUANTITY_INDEX(IQ))%INTEGRATED_PARTICLES) P_FLUX = .TRUE.
   ENDDO
ENDIF

IF (P_FLUX .AND. .NOT.EVACUATION_ONLY(NM)) CALL COMPUTE_PARTICLE_FLUXES(NM)
...
QUANTITY_LOOP: DO IQ=1,NQT
   ...
   IF (.NOT. AGL_TERRAIN_SLICE) THEN
      DO K=KK1,KK2
         DO J=JJ1,JJ2
            DO I=II1,II2
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,K,IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,PROP_INDEX,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ELSE
      ...
      DO I=II1,II2
         DO J=JJ1,JJ2
            DO K=KK1,KK2
               ...
               QUANTITY(I,J,K) = GAS_PHASE_OUTPUT(I,J,KTS,IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,T,NM)
            ENDDO
         ENDDO
      ENDDO
   ENDIF
...
REAL(EB), INTENT(IN) :: T
REAL(EB) :: VALUE,VALUE2,STAT_VALUE,SUM_VALUE,VOL,WGT_LINE,T_TMP,DVAL,DVAL2,DEV2,WGT_UNBIASED_LINE,WGT
INTEGER :: NM,N,I,J,K,STAT_COUNT,IW,SURF_INDEX,I_STATE,IND,LP_INDEX,LOCATION_INDICES(3)
LOGICAL :: NOT_FOUND,P_FLUX
 
CALL POINT_TO_MESH(NM)

! Weighting factor for running average of line devices

WGT_LINE = DT/MAX(DT,T-(T_END-DT_DEVC_LINE))
WGT_UNBIASED_LINE = 1._EB/WGT_LINE-1._EB
IF (WGT_UNBIASED_LINE>TWO_EPSILON_EB) THEN
   WGT_UNBIASED_LINE = 1._EB/WGT_UNBIASED_LINE
ELSE
   WGT_UNBIASED_LINE = WGT_LINE
ENDIF
...
DO N=1,N_DEVC
   DV => DEVICE(N)
   IF (OUTPUT_QUANTITY(DV%OUTPUT_INDEX)%INTEGRATED_PARTICLES) P_FLUX = .TRUE.
ENDDO
         ...
         GAS_STATS_SELECT: SELECT CASE(DV%STATISTICS)
            ...
            CASE('null','RMS','TIME MIN','TIME MAX') GAS_STATS_SELECT
               ...
               VALUE = GAS_PHASE_OUTPUT(DV%I,DV%J,DV%K,DV%OUTPUT_INDEX,0,DV%Y_INDEX,DV%Z_INDEX,DV%PART_INDEX,DV%VELO_INDEX,&
                                        DV%PIPE_INDEX,DV%PROP_INDEX,T,NM)
   ...
   ELSE DV_LINE_IF ! DV%LINE > 0
      ...
      SELECT CASE(TRIM(DV%STATISTICS))
         CASE('RMS') ! root mean square
            DV%RMS_VALUE = (1._EB-WGT_UNBIASED_LINE)*DV%RMS_VALUE + WGT_UNBIASED_LINE*(VALUE-DV%AVERAGE_VALUE)**2
         ...
         CASE('COV') ! covariance
            ...
            DV%COV_VALUE = (1._EB-WGT_UNBIASED_LINE)*DV%COV_VALUE + WGT_UNBIASED_LINE*DVAL*DVAL2
         ...
         CASE('CORRCOEF') ! correlation coefficient
            ...
            DV%COV_VALUE  = (1._EB-WGT_UNBIASED_LINE)*DV%COV_VALUE  + WGT_UNBIASED_LINE*DVAL*DVAL2
            DV%RMS_VALUE  = (1._EB-WGT_UNBIASED_LINE)*DV%RMS_VALUE  + WGT_UNBIASED_LINE*(VALUE -DV%AVERAGE_VALUE )**2
            DV%RMS_VALUE2 = (1._EB-WGT_UNBIASED_LINE)*DV%RMS_VALUE2 + WGT_UNBIASED_LINE*(VALUE2-DV%AVERAGE_VALUE2)**2
...
REAL(EB) RECURSIVE FUNCTION GAS_PHASE_OUTPUT(II,JJ,KK,IND,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,T,NM) &
...
REAL(EB), INTENT(IN) :: T
INTEGER, INTENT(IN) :: II,JJ,KK,IND,IND2,NM,VELO_INDEX,PIPE_INDEX,PROP_INDEX
...
SELECT CASE(IND)
   ...
   CASE(21)  ! RELATIVE HUMIDITY
      ...
      ELSE
         ...
         GAS_PHASE_OUTPUT_RES = RELATIVE_HUMIDITY(Y_ALL(H2O_INDEX),TMP(II,JJ,KK),PBAR(0,PRESSURE_ZONE(II,JJ,KK)))
      ENDIF
   CASE(22)  ! HS
   ...
   CASE(36)  ! POTENTIAL TEMPERATURE
      GAS_PHASE_OUTPUT_RES = GET_POTENTIAL_TEMPERATURE(TMP(II,JJ,KK),II,JJ,KK,NM)
   ...
   CASE(52)  ! WEM (wavelet error measure)
      GAS_PHASE_OUTPUT_RES = WAVELET_ERROR_MEASURE(II,JJ,KK,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,NM)
   ...
   CASE(56)  ! SUBGRID KINETIC ENERGY (per unit mass)
      ...
      GAS_PHASE_OUTPUT_RES = SUBGRID_KINETIC_ENERGY(MU(II,JJ,KK),RHO(II,JJ,KK),C_DEARDORFF,DELTA)
   ...
   CASE(85)  ! KOLMOGOROV LENGTH SCALE
      SS = GAS_PHASE_OUTPUT(II,JJ,KK,84,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,T,NM)
   ...
   CASE(86)  ! CELL REYNOLDS NUMBER
      ...
      KKK = MAX(1,MIN(KK,KBAR))
      DELTA = LES_FILTER_WIDTH_FUNCTION(DX(III),DY(JJJ),DZ(KKK))
      ETA = GAS_PHASE_OUTPUT(II,JJ,KK,85,IND2,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,PIPE_INDEX,PROP_INDEX,T,NM)
   ...
   CASE(130) ! Extinction
      ...
      DO NR=1,N_REACTIONS
       ...
       IF (ZZ_FUEL > ZZ_MIN_GLOBAL .AND. ZZ_OX > ZZ_MIN_GLOBAL .AND. Q(II,JJ,KK) == 0._EB) GAS_PHASE_OUTPUT_RES = 0._EB

   CASE(131) ! CHEMISTRY SUBITERATIONS
      GAS_PHASE_OUTPUT_RES = CHEM_SUBIT(II,JJ,KK)
   
   CASE(132) ! REACTION SOURCE TERM
   ...
   CASE(135) ! MACH NUMBER
      ...
      GAMMA = CP/(CP-RCON)
      GAS_PHASE_OUTPUT_RES = SQRT(2._EB*KRES(II,JJ,KK))/SQRT(RCON*TMP(II,JJ,KK)*GAMMA)
...
REAL(EB) :: CONCORR,FLUX,DFLUXDT,SOLID_PHASE_OUTPUT_OLD,VOLSUM,C_S_ADJUST_UNITS,MFT,&
            ZZ_GET(1:N_TRACKED_SPECIES),Y_SPECIES,KSGS,DEPTH,UN,H_S
...
SOLID_PHASE_SELECT: SELECT CASE(INDX)
   ...
   CASE(13) ! GAUGE HEAT FLUX
      ...
      SOLID_PHASE_OUTPUT = (ONE_D%QRADIN/(ONE_D%EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 + &
   ...
   CASE(22) ! RADIOMETER
      SOLID_PHASE_OUTPUT = ( ONE_D%QRADIN/(ONE_D%EMISSIVITY+1.0E-10_EB) - SIGMA*PY%GAUGE_TEMPERATURE**4 )*0.001_EB
   CASE(23) ! ADIABATIC SURFACE TEMPERATURE
      SOLID_PHASE_OUTPUT_OLD = ONE_D%TMP_F
      SOLID_PHASE_OUTPUT     = ONE_D%TMP_F
      ADLOOP: DO
         CONCORR = ONE_D%HEAT_TRANS_COEF*(ONE_D%TMP_F-SOLID_PHASE_OUTPUT_OLD)      
         FLUX = ONE_D%QCONF + CONCORR + ONE_D%QRADIN - ONE_D%EMISSIVITY * SIGMA * SOLID_PHASE_OUTPUT_OLD ** 4
         DFLUXDT = -4._EB*ONE_D%EMISSIVITY*SIGMA*SOLID_PHASE_OUTPUT_OLD**3 - ONE_D%HEAT_TRANS_COEF
         IF (ABS(DFLUXDT)>=TWO_EPSILON_EB) SOLID_PHASE_OUTPUT_OLD = SOLID_PHASE_OUTPUT_OLD - FLUX / DFLUXDT
   ...
   CASE(33) ! SOLID SPECIFIC HEAT
      ...
      MATERIAL_LOOP_CP: DO NN=1,SF%N_MATL
         ...
         ELSE
            NR     = -NINT(ML%C_S)
            SOLID_PHASE_OUTPUT = SOLID_PHASE_OUTPUT + ONE_D%RHO(II1,NN)*EVALUATE_RAMP(ONE_D%TMP(II1),0._EB,NR)* &
...
DO NM=1,NMESHES
   IF (HRR_TIME_INTERVAL(NM)>0._EB) THEN
      Q_DOT_TOTAL(:) = Q_DOT_TOTAL(:) + Q_DOT_SUM(:,NM)/HRR_TIME_INTERVAL(NM)
      M_DOT_TOTAL(1) = M_DOT_TOTAL(1) + M_DOT_SUM(1,NM)/HRR_TIME_INTERVAL(NM)
      M_DOT_TOTAL(2) = M_DOT_TOTAL(2) + M_DOT_SUM(2,NM)/HRR_TIME_INTERVAL(NM)
   ENDIF
ENDDO
...
DO N=1,N_ISOF
   ...
   IF (EX) THEN
      FLUSH(ABS(LU_ISOF(N,NM)))
   ENDIF
ENDDO

DO N=1,4
   INQUIRE(FILE=FN_SMOKE3D(N,NM),EXIST=EX)
   IF (EX) THEN
      FLUSH(ABS(LU_SMOKE3D(N,NM)))
   ENDIF
END DO  
...
SS(1) = GAS_PHASE_OUTPUT(MAX(0,II-2),JJ,KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(2) = GAS_PHASE_OUTPUT(MAX(0,II-1),JJ,KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(4) = GAS_PHASE_OUTPUT(MIN(MESHES(NM)%IBP1,II+1),JJ,KK,IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
WAVELET_ERROR_MEASURE = WAVELET_ERROR(SS)

IF (.NOT.TWO_D) THEN
   SS(1) = GAS_PHASE_OUTPUT(II,MAX(0,JJ-2),KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
   SS(2) = GAS_PHASE_OUTPUT(II,MAX(0,JJ-1),KK,              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
   SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
   SS(4) = GAS_PHASE_OUTPUT(II,MIN(MESHES(NM)%JBP1,JJ+1),KK,IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
   WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
ENDIF

SS(1) = GAS_PHASE_OUTPUT(II,JJ,MAX(0,KK-2),              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(2) = GAS_PHASE_OUTPUT(II,JJ,MAX(0,KK-1),              IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(3) = GAS_PHASE_OUTPUT(II,JJ,KK,                       IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
SS(4) = GAS_PHASE_OUTPUT(II,JJ,MIN(MESHES(NM)%KBP1,KK+1),IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
WAVELET_ERROR_MEASURE = MAX(WAVELET_ERROR_MEASURE,WAVELET_ERROR(SS))
...
DO K=KK-1,KK+1
   ...
   DO J=JJ-1,JJ+1
      ...
      DO I=II-1,II+1
         ...
         ZZ(I-II+2,J-JJ+2,K-KK+2) = GAS_PHASE_OUTPUT(LL,MM,NN,IND,0,Y_INDEX,Z_INDEX,PART_INDEX,VELO_INDEX,0,0,0._EB,NM)
      ENDDO
   ENDDO
ENDDO
...
WRITE(MODULE_DATE,'(A)') dumprev(INDEX(dumprev,':')+2:LEN_TRIM(dumprev)-2)
