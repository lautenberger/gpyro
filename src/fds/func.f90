...
IMPLICIT NONE 
CHARACTER(255), PARAMETER :: funcid='$Id: func.f90 22187 2015-04-01 15:26:22Z mcgratta $'
CHARACTER(255), PARAMETER :: funcrev='$Revision: 22187 $'
CHARACTER(255), PARAMETER :: funcdate='$Date: 2015-04-01 08:26:22 -0700 (Wed, 01 Apr 2015) $'
...
WC%NOM                => OS%INTEGERS(12,STORAGE_INDEX) ; IF (NEW) WC%NOM = 0
WC%NIC_MIN            => OS%INTEGERS(13,STORAGE_INDEX) ; IF (NEW) WC%NIC_MIN = 0
WC%NIC_MAX            => OS%INTEGERS(14,STORAGE_INDEX) ; IF (NEW) WC%NIC_MAX = 0
...
WC%Q_LEAK             => OS%REALS(18,STORAGE_INDEX) ; IF (NEW) WC%Q_LEAK = 0._EB
WC%TMP_F_GAS          => OS%REALS(19,STORAGE_INDEX) ; IF (NEW) WC%TMP_F_GAS = TMPA
...
ONE_D%MASSFLUX_ACTUAL(1:I2-I1+1) => OS%REALS(I1:I2,STORAGE_INDEX) 
IF (NEW) ONE_D%MASSFLUX_ACTUAL(1:I2-I1+1) = 0._EB
...
!IF (NEW) ONE_D%X(0:SF%N_CELLS_INI) = SF%X_S(0:SF%N_CELLS_INI) !CWL: Replaced this line with below
IF (NEW .AND. .NOT. (SF%GPYRO)) ONE_D%X(0:SF%N_CELLS_INI) = SF%X_S(0:SF%N_CELLS_INI)
...
!IF (NEW) ONE_D%RHO(0:SF%N_CELLS_INI+1,1:SF%N_MATL) = SF%RHO_0(0:SF%N_CELLS_INI+1,1:SF%N_MATL) !CWL: Replaced this with line below
IF (NEW .AND. .NOT. (SF%GPYRO)) ONE_D%RHO(0:SF%N_CELLS_INI+1,1:SF%N_MATL) = SF%RHO_0(0:SF%N_CELLS_INI+1,1:SF%N_MATL)
...
REAL(EB) :: C_P_G,C_P_L,CP,CP2,DCPDT,DHOR,E_TOT,H_V,M_G_NEW,M_L_NEW,MW_G,MW_RATIO,MW_V,M_ZZ(1:N_TRACKED_SPECIES),M_V0,RESIDUAL,&
            TMP_1,TMP_2,TMP_G,X,X_EQ,X_MAX,Y_ALL(1:N_SPECIES),Y_EQ_2,ZZ_NEW(1:N_TRACKED_SPECIES)
