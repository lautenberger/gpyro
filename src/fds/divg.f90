...
IMPLICIT NONE
PRIVATE
CHARACTER(255), PARAMETER :: divgid='$Id: divg.f90 22080 2015-03-21 21:01:50Z randy.mcdermott $'
CHARACTER(255), PARAMETER :: divgrev='$Revision: 22080 $'
CHARACTER(255), PARAMETER :: divgdate='$Date: 2015-03-21 14:01:50 -0700 (Sat, 21 Mar 2015) $'
...
!$OMP END PARALLEL DO
! Begin CWL addition for local igniter (volumetric heat source)
IF (T .GE. TIGNSTART .AND. T .LE. TIGNSTOP) THEN
   DO K=1,KBAR
   DO J=1,JBAR
   DO I=1,IBAR
	  IF (X(I) .GE. IGNLOC(1) .AND. X(I) .LE. IGNLOC(2) ) THEN
      IF (Y(J) .GE. IGNLOC(3) .AND. Y(J) .LE. IGNLOC(4) ) THEN
      IF (Z(K) .GE. IGNLOC(5) .AND. Z(K) .LE. IGNLOC(6) ) THEN
         Q(I,J,K) = Q(I,J,K) + IGNPOW
      ENDIF
      ENDIF
      ENDIF
   ENDDO
